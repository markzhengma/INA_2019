...
```
Eliminating Overlaps Between Keyword Vectors for More Unique Identifiers
```{r}
uniqueCSKeywords <- setdiff(computerScience, chemistry)
uniqueCSKeywords <- setdiff(uniqueCSKeywords, physics)

uniqueCHEMKeywords <- setdiff(chemistry, computerScience)
uniqueCHEMKeywords <- setdiff(uniqueCHEMKeywords, physics)

uniquePHYSKeywords <- setdiff(physics, computerScience)
uniquePHYSKeywords <- setdiff(uniquePHYSKeywords, chemistry)
```
...
Converting to a CSV
```{r}
write.csv(uniqueCSKeywords, "uniqueCSKeywords.csv")
write.csv(uniqueCHEMKeywords, "uniqueCHEMKeywords.csv")
write.csv(uniquePHYSKeywords, "uniquePHYSKeywords.csv")

```

Testing the Model for CS Accuracy
```{r}
testCSFiles <- list.files(path = "/Users/juandjuwadi/Documents/INATopics/Testing/CS Papers")
testCSModel <- function(files){
  correct = 0
  total = length(files)
  for (n in files){
  text = pdf_text(paste("/Users/juandjuwadi/Documents/INATopics/Testing/CS Papers/",n, sep = ""))
  ...
  print(paste("Accuracy is ", round((correct/total) * 100)))
}

testCHFiles <- list.files(path = "/Users/juandjuwadi/Documents/INATopics/Testing/Chem Papers")
testCHModel <- function(files){
  correct = 0
  total = length(files)
  for (n in files){
  text = pdf_text(paste("/Users/juandjuwadi/Documents/INATopics/Testing/Chem Papers/",n, sep = ""))
  topic <- topicModel(text)
  if (is.element(topic, "Chemistry")){
    correct = correct + 1
  }
  print(topic)
  }
